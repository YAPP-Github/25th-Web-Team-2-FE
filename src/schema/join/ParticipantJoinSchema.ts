...
const ParticipantJoinSchema = () => {
  return z.object({
    ...
    contactEmail: z
      ...
      .nonempty({ message: '연락 받을 이메일을 입력해주세요' }),
    ...
    // 성별. 남자/여자/선택안함
    gender: z.union([z.literal('MALE'), z.literal('FEMALE'), z.literal('ALL')]),
    ...
    // 거주 지역. 시.도(region), 시.군.구(area)
    basicAddressInfo: z.object({
      region: z.string().nonempty({ message: '거주 지역의 시·도를 입력해주세요' }),
      area: z.string().nonempty({ message: '거주 지역의 시·군·구를 입력해주세요' }),
    }),

    // 추가 활동 지역. 선택. 시.도(region), 시.군.구(area)
    additionalAddressInfo: z
      .object({
        region: z.string().nonempty({ message: '추가 활동 지역의 시·도를 입력해주세요' }),
        area: z.string().nonempty({ message: '추가 활동 지역의 시·군·구를 입력해주세요' }),
      })
      .optional(),
